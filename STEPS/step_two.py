...
def	check_step_two(c):
	...
	well_placed += cube.check_face_rows(c.cube[0], 0, 1)
	if c.cube[0][1][1] == 0:
		well_placed += 1
	# Partie check face Right (Red)
	well_placed += cube.check_face_rows(c.cube[1], 1, 1)
	if c.cube[1][1][1] == 1:
		well_placed += 1
	# Partie check face Back (Green)
	well_placed += cube.check_face_rows(c.cube[3], 3, 1)
	if c.cube[3][1][1] == 3:
		well_placed += 1
	# Partie check face Left (Orange)
	well_placed += cube.check_face_rows(c.cube[4], 4, 1)
	if c.cube[4][1][1] == 4:
		well_placed += 1
